...
DeepTutor 当前内置 32 个 LLM provider 绑定、12 个 embedding provider 绑定和 7 个激活的搜索 provider（外加显式的 `none` 选项）。可以通过 Web 设置页面、`deeptutor init`、`data/user/settings/model_catalog.json` 或进程环境变量来配置。项目根目录的 `.env` 文件**不会**被运行时自动加载。
